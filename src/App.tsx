...
export type DateFilterType = 'all' | 'today' | 'this-week' | 'this-month' | 'this-year' | 'custom';
...
function App() {
  ...
  useEffect(() => {
    localStorage.setItem('genz-todos', JSON.stringify(todos));
  }, [todos]);
  ...
  const isDateInRange = (date: Date) => {
    ...
    switch (dateFilter) {
      ...
      case 'custom':
        return todoDate.getMonth() === selectedMonth && todoDate.getFullYear() === selectedYear;
      default:
        return true;
    }
  };
  ...
  const filteredTodos = todos.filter(todo => {
    ...
    // Apply date filter
    const passesDateFilter = isDateInRange(todo.createdAt);
  ...
  const availableYears = [...new Set(todos.map(todo => new Date(todo.createdAt).getFullYear()))].sort((a, b) => b - a);
  const availableMonths = [...new Set(todos.filter(todo => new Date(todo.createdAt).getFullYear() === selectedYear).map(todo => new Date(todo.createdAt).getMonth()))].sort((a, b) => a - b);
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        ...
        >
          ...
          <motion.p
            ...
            className={`text-lg ${darkMode ? 'text-white/80' : 'text-gray-700'} font-medium`}
          >
        ...
        {/* Progress Bar */}
        <ProgressBar completed={completedCount} total={totalCount} darkMode={darkMode} />
              ...
              {[
                ...
                >
                  ...
                  <span className="relative z-10 flex items-center justify-center gap-1">
                    ...
                    <span className="sm:hidden">{dateTab.label.split(' ')[0]}</span>
                  </span>
                </motion.button>
              ))}
            </div>
